...
export class OpenAIProvider implements IAIProvider {
  ...
  public async generate(request: IAIRequest): Promise<IAIResponse> {
    const providerConfig = aiConfig.providers.openai;
    const apiKey = providerConfig.apiKey;
    if (!apiKey) {
      throw new AuthenticationError("OpenAI API key is not configured.");
    }
    ...
    logger.info("[AI] OpenAI request started", {
      provider: this.id,
      model,
      timeoutMs,
    });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error("[AI] OpenAI HTTP error", {
          provider: this.id,
          status,
          providerCode,
          rawBody,
      ...
      if (!responseText || responseText.trim().length === 0) {
        throw new InvalidAIResponseError("OpenAI returned an empty response");
      }
      if (responseText.trim().length < minLength) {
        throw new InvalidAIResponseError(
          `OpenAI response too short (${responseText.trim().length} chars, min ${minLength})`
        );
      }
      if (responseSizeBytes > maxSizeBytes) {
        throw new InvalidAIResponseError(
          `OpenAI response exceeds max allowed size (${responseSizeBytes} bytes, max ${maxSizeBytes})`
        );
      }
    ...
    try {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          else if (httpStatus === 429 || lc.includes("rate limit") || lc.includes("quota")) { errorType = lc.includes("quota") ? "Quota Exceeded" : "Rate Limited"; }

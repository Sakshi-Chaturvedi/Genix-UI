...
export class OpenRouterProvider implements IAIProvider {
  ...
  public async generate(request: IAIRequest): Promise<IAIResponse> {
    ...
    if (!apiKey) {
      throw new AuthenticationError("OpenRouter API key is not configured.");
    }
    ...
    logger.info("[AI] OpenRouter request started", {
      provider: this.id,
      model,
      timeoutMs,
    });
    ...
    try {
      const response = await fetch(endpoint, {
        ...
        headers: {
          ...
          "HTTP-Referer": "https://github.com/Genix-UI",
          "X-Title": "Genix UI",
        },
        body: JSON.stringify({
          model,
      ...
      if (!response.ok) {
        ...
        // Try to extract structured provider error fields
        ...
        logger.error("[AI] OpenRouter HTTP error", {
          provider: this.id,
          status,
          providerCode,
          rawBody,
      ...
      if (!responseText || responseText.trim().length === 0) {
        throw new InvalidAIResponseError("OpenRouter returned an empty response");
      }
      if (responseText.trim().length < minLength) {
        throw new InvalidAIResponseError(
          `OpenRouter response too short (${responseText.trim().length} chars, min ${minLength})`
        );
      }
      if (responseSizeBytes > maxSizeBytes) {
        throw new InvalidAIResponseError(
          `OpenRouter response exceeds max allowed size (${responseSizeBytes} bytes, max ${maxSizeBytes})`
        );
      }
  ...
  public async healthCheck(): Promise<any> {
    ...
    try {
      ...
      try {
        const response = await fetch(endpoint, {
          method: "POST",
          headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}`, "HTTP-Referer": "https://github.com/Genix-UI", "X-Title": "Genix UI" },
        ...
        if (!response.ok) {
          ...
          else if (httpStatus === 404 || lc.includes("no endpoints found") || lc.includes("not found")) errorType = "Model Not Found";
